...
ACCOUNT_CSV = """
    group, account, currency, vat_code, description
...
def set_up_ledger_and_account():
    ...
    initial_account_chart = cashctrl.accounts()
    initial_ledger = cashctrl.ledger()

    # Create test accounts and Ledger
    cashctrl.mirror_accounts(TEST_ACCOUNTS, delete=False)
    ...
    cashctrl.mirror_accounts(initial_account_chart, delete=True)
...
def test_FX_revaluation(set_up_ledger_and_account):
    ...
    params = {"from": from_currency, "to": cashctrl.reporting_currency, "date": None}
