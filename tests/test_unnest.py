...
import pytest
import pandas as pd
from cashctrl_ledger import nest, unnest
...
def test_unnest_with_matching_columns_in_inner_and_outer_dfs():
    df = pd.DataFrame({
        'id': [10, 16],
        'text': ['hello', 'world'],
        'items': [pd.DataFrame({'id': [33, 33], 'sub_text': ['test1', 'test2']}),
...
def test_successive_unnest_and_nest_returns_original_df():
    data = {
        'id': [10, 16],
        'text': ['hello', 'world'],
        'data': [pd.DataFrame({'sub_id': [33, 33], 'sub_text': ['test1', 'test2']}),
                 pd.DataFrame({'sub_id': [20, 16], 'sub_text': ['test3', 'test4']})]
    }
    df = pd.DataFrame(data)
...
def test_unnest_with_non_dataframe_objects():
    data = {
        'id': [10, 16],
        'text': ['hello', 'world'],
        'test': [pd.DataFrame({'sub_id': [33, 33], 'sub_text': ['test1', 'test2']}),
